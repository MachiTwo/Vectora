...
- `vectora/`: produto principal, incluindo backend Python (FastAPI + LangChain), frontend React e CLI
- `vectora-asset-library/`: biblioteca/registry de assets e datasets
- `vectora-cognitive-runtime/`: VCR, o mecanismo de decisão em Python (PyTorch + XLM-RoBERTa)
- `vectora-integrations/`: integrações e SDKs
...
- **Código**: todo o código-fonte Python no monorepo deve estar em inglês (nomes de variáveis, comentários, docstrings).
...
2. Adira às convenções padrão de Python (PEP 8, type hints) ao modificar o código em `vectora/` e `vectora-cognitive-runtime/`.
...
4. **Verificação**: executar testes (`uv run pytest tests/`), linting (`uv run ruff check .`) e formatação (`uv run ruff format .`) após cada fase.
